...
import com.grapeqin.project.core.ResultCode;
import com.grapeqin.project.core.ServiceException;
...
public class MyWebMvcConfigurer implements WebMvcConfigurer {
	...
	private final Logger logger =
			LoggerFactory.getLogger(MyWebMvcConfigurer.class);
	...
	/**
	 * 一个简单的签名认证，规则： 1. 将请求参数按ascii码排序 2. 拼接为a=value&b=value..
	 * .这样的字符串（不包含sign） 3.
	...
	private boolean validateSign(HttpServletRequest request) {
		...
		// =19e907700db7ad91318424a97c54ed57
		if (StringUtils.isEmpty(requestSign)) {
			return false;
		}
		List<String> keys =
				new ArrayList<String>(request.getParameterMap().keySet());
		// 排除sign参数
		keys.remove("sign");
		// 排序
